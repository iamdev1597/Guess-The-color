...
const GetRound = () => {
    ...
    if (Number(round) && round != 0) {
        ...
        alert(`How can you play ${round} round`);        
    }
}
...
const CheckAnwser = (anwser) => {
    ...
    if (round_count != round) {
        for (let i = 0; i < Check.length - 1; i++) {
            ...
            if (i == 2) {
                wrong += 1;
                try_count += 1;
                alert("Oh! Better luck next time")
            }
        }
        $("#label").text(`Round ${round_count + 1}`);
    ...
    if (round_count == round) {
        setTimeout(function(){
            ...
            document.querySelector("#rate").innerHTML += `<label>Incorrect Attempt : ${wrong}</label>`;
        }, 3000);        
    }
}
